...
class NatNetClient:
    ...
    def __unpack_marker_set_description(
        self, bytestream: bytes, NatNetStreamVersion: List[int] = None
    ) -> int:
        # offset += 4
    ...
    def __unpack_force_plate_description(
        ...
        force_plate_desc = forcePlateDescription(
            bytestream, offset, NatNetStreamVersion
        )
        ...
        while not stop():
            ...
            if not self.settings["use_multicast"] and not stop():
                self.send_keep_alive(
                    in_socket, self.settings["server_ip"], self.settings["command_port"]
                )
