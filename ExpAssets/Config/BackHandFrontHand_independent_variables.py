...
BackHandFrontHand_ind_vars = IndependentVariableSet()
BackHandFrontHand_ind_vars.add_variable("arrangement", str, (
...
>> BackHandFrontHand_ind_vars.add_variable("color", str)
>> BackHandFrontHand_ind_vars.add_variable("size", float)
>> BackHandFrontHand_ind_vars.add_variable("active", bool)
>> BackHandFrontHand_ind_vars.add_variable("count", int)

Finally, we add values to each variable. This can be done one at a time, as in the colors example below:

>> BackHandFrontHand_ind_vars['color'].add_value("blue")
>> BackHandFrontHand_ind_vars['color'].add_value("blue")
>> BackHandFrontHand_ind_vars['color'].add_value("blue")

Or altogether in a comma-separated set, as in the 'count' example:

>> BackHandFrontHand_ind_vars['count'].add_values(1,2,3,4,5)
...
>> BackHandFrontHand_ind_vars['size'].add_value(1.0, 2)
>> BackHandFrontHand_ind_vars['size'].add_value(2.0, 2)
>> BackHandFrontHand_ind_vars['size'].add_value(5.0)
...
>> BackHandFrontHand_ind_vars['active'].add_values((True, 5), False)

which is equivalent to:

>> BackHandFrontHand_ind_vars['active'].add_value(True, 5)
>> BackHandFrontHand_ind_vars['active'].add_value(False)
...
>> BackHandFrontHand_ind_vars['size'].enabled = False
...
>> BackHandFrontHand_ind_vars['size'][1.0].enabled = False
